...
- 任务状态由 [current-work.md](../state/current-work.md) 跟踪；当前运行状态以 [status.md](../project/status.md) 为唯一摘要。
...
- 内核补丁必须分别通过离线 DKMS 编译与实机重启验证，结果写入对应 `docs/patches/` 记录；
  失败项进入 [current-work.md](../state/current-work.md) 或 [suspended.md](../state/suspended.md)。
