...
> **安装前无需卸载 patched-27**：两包同名 `innogpu-fh2m-trixie` 且 `4.0.0-i1 > 3.3.3.42-patched-27`，
> 安装即同包名升级。旧包 prerm 自动执行 `dkms remove -m innogpu-kernel -v 2.2 --all`，新包 postinst
> 自动 `dkms add/build/install --force`，DKMS 状态无缝交接（maintainer scripts 字节一致已验证）。
> 先卸载反而制造无驱动失败窗口；正在运行的模块不热卸载，重启后加载新模块（对齐重启纪律）。
