...
| power-loss：回滚还原 mv 后、清理完成前断电 | 回滚还原 mv + fsync_dir(OUT_DIR) + fsync_dir(.txn) 完成后、rm journal / rm -rf .txn 前强制断电 | journal=rolling_back + OUT_DIR 完整旧对（目标目录项持久）+ .txn 无 old.*（源目录删除持久 → **无双存在**）→ 重入回滚段：无 new 文件可删、无 old.* 可还原 → 自校验 PASS → 清理 → exit 1；OUT_DIR = 完整旧对（v20 per codex v19 P1 #1） |
