...
- 日期：2026-09-12
- 状态：**R5 停批，i5 两探针已返回并健康恢复，030-034/i6 stop-stage 实现批待初审/终审（2026-09-16）**。当前 builder 消费 18 链 i6 隔离快照；新状态继续使用独立 devres，共享 ABI 不变。恢复证明与执行约束见 [诊断设计 §11](r5-suspend-030-032-debug-design.md)。下文各代传导记录为历史锚点；不创建 validation-results、不签发、不打 tag。
