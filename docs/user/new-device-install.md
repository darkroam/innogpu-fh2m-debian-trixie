...
| 版本 | 用途 | 新设备策略 |
| --- | --- | --- |
| `5.0.0-iN` | **当前主线（fantgpu 血缘，诊断线）**：F0 + 030-NNN 链（当前 i6）；R5=FAIL 悬案未解除 | 非新设备默认版本；R5 未定位前不推广；`postinst_current_kernel_only` 发布阻断未解除 |
| `4.0.2-i3` | **回退基线（历史交付）**：i2 + patch-029 DDCCI panel 创建恢复（继承 patch-024/026/028）；SHA `177133ee…` | 当前回滚卡；R14 6/6 deep 通过；新设备安装后仍须先做图形基线和受控 suspend 验收 |
