...
当前设备运行 `5.0.0-i6`（诊断线）；**首选回退 = 4.0.2-i3（回滚卡）**，随后是 `4.0.0-i1`、patched-27（见下节）。patched-17 是深层保守回退点，
patched-8 只在 patched-17 仍不能启动时使用。执行任何升级前都应把 `patched-27`、`patched-17`、
`patched-8` 三个 deb 放入 `debs/`，并保留 SSH 或真实 TTY 恢复通道。

历史事故：`4.0.1-i1` 于 2026-09-02 安装并在 s2idle 唤醒后出现外屏红屏，当时整体回退 `4.0.0-i1`
并重启验证；不得再次安装该失败候选。以下分段流程必须匹配实际回退起点。
