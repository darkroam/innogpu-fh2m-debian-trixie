...
| 范围 | 入口与用例 | 小计 |
| --- | --- | --- |
| unit | manifest 9 + license 50 + version 9 + extractor 7 + results parser 19 + exec probes 12 + VA-API 56 + DMA-BUF 147 + DRI repair 34 + collab 26 + suspend/resume 24 | 11 个入口 / 393 项 |
| 其他 CI/沙箱 | fbterm 1 + package 11 + Picom install 3 + Picom session 3 + xdisplay 5 | 5 个入口 / 23 项 |
| **合计** | 不含需真实设备/root/副作用授权的 runtime 项 | **16 个入口 / 416 项** |
...
| suspend/resume 静态 | tests/unit/run-suspend-resume-tests.sh | patch-024/025 dry-run/应用、025 三行上下文、OFF 门禁位于 OPP/PreClock 前、post-atomic 重复光标恢复移除、单文件范围、`4.0.1-i3`/`4.0.1-i4` 共用 epoch 且旧迭代失败关闭、零 fuzz/无备份应用、编译树与包源码双扫描、patched-28 legacy 接线、R07 观测器无挂起动作/ABI 失败关闭/只记录自然驱动活动 | shell/python/patch，复制跟踪源码到 `/tmp` | 否 |
