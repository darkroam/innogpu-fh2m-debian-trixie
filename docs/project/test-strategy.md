...
| 范围 | 入口与用例 | 小计 |
| --- | --- | --- |
| unit | manifest 9 + license 50 + version 11 + extractor 7 + results parser 19 + exec probes 12 + VA-API 56 + DMA-BUF 147 + DRI repair 34 + collab 26 + suspend/resume 53 + suspend failure finalize 15 | 12 个入口 / 439 项 |
...
| suspend/resume 静态 | tests/unit/run-suspend-resume-tests.sh | patch-024/025-display/026-lifecycle/028-temp-monitor dry-run/应用、OFF 快速门禁、devfreq drain→PVR suspend、PVR/DVFS resume→温度 work 锁序、多父/多子计数、结构尾部 ABI、失败/旧内核/NO_HARDWARE、版本/epoch 失败关闭、零 fuzz/无备份应用、编译树与包源码双扫描、patched-28 legacy 接线、R08 只读观测契约 | shell/python/patch，复制跟踪源码到 `/tmp` | 否 |
