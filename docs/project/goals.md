...
3. **可维护可演进**：从"厂商黑盒 + 补丁叠加"逐步走向"自有源码树 + 清单管理二进制"，代码可迭代、
   可测试、可复现（见 [source-tree-migration.md](../planning/source-tree-migration.md)）；
...
5. **源码树迁移**：阶段 0（设计）已完成；阶段 1 导入 `drivers/` + 补丁转提交 + 二进制清单与
   幂等提取工具 + 新构建流程；阶段 2 独立版本号 + 开发测试闭环；阶段 3 在源码树内重构。
