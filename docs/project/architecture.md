...
`innogpu-fh2m-trixie 3.3.3.42-patched-22` 是当前设备已安装并重启、完成 connector/桌面烟测的版本；
...
和本地 connector 修复，并保留同一原包中的完整用户态 ABI 集合。patched-21 使用
`patch-006`；当前 patched-22 另加 `patch-009`，仅针对本机 DPU match 141 的内置 DP0/eDP 语义。
