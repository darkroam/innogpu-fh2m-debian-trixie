...
| `patches/` | 历史补丁 provenance 与新行为修复候选；已迁入源码树的补丁不重复叠加，R11 `4.0.2-i1` 绑定 patch-024 + patch-026-suspend-resume-dvfs-lifecycle，R06 i3/i4 保留为历史实验入口 |
...
只应用 patch-024 + lifecycle 026，不含 display 025；后续每次行为变化仍必须独立升号。patched-21 使用
...
当前构建器默认接受 `4.0.2-i1` 与固定 epoch `1788624000`：在 patch-024 后应用
patch-026-suspend-resume-dvfs-lifecycle，不含 UNVERIFIED 的 display 025。补丁同时应用到离线编译
staging 和最终包内 DKMS 源码，并拒绝 `.orig/.rej/.o.cmd` 产物。R06 的 `4.0.1-i3/i4` 及共同
epoch `1788451200` 仍可显式复现；i1/i2 不再由当前源码复用。当前运行版本是回退后的 `4.0.0-i1`。
...
i3/i4 对照；R11 lifecycle 026 用 Debian 6.12 devfreq 的同步 suspend 语义关闭已证明的 TOCTOU，
但仍需真机 deep 验收。旧
