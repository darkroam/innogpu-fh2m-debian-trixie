...
static T_TimingMap aTimingTable[] =
{
...
static const char *ConvertTiming2Name(int i_nVic)
{
	int i = 0;
...
static int Dtd2Vic(struct seq_file *m, unsigned char *i_pTable)
{
	int i = 0;
	int j = 0;
	int s32Vic = 0;
...
static int DtdPrint(struct seq_file *m, unsigned char *i_pDtd)
{
	int htotal = 0;
	int vtotal = 0;
	...
	fh2m_inno_seq_printf(m, "\tMode \t\"%dx%d\"\n", H_ACTIVE, V_ACTIVE );
//	fh2m_inno_seq_printf(m, "\t# vfreq %ldHz, hfreq %ldkHz\n", (long)PIXEL_CLOCK/((long)vtotal*(long)htotal),
//		(long)PIXEL_CLOCK/(long)(htotal*1000));
//	fh2m_inno_seq_printf(m, "\t\t DotClock\t%ld\n", (long)PIXEL_CLOCK/1000000);
	fh2m_inno_seq_printf(m, "\t\t HTimings\t%u %u %u %u\n", H_ACTIVE, H_FRONT_PORCH, H_SYNC_WIDTH, htotal);
	fh2m_inno_seq_printf(m, "\t\t VTimings\t%u %u %u %u\n", V_ACTIVE, V_FRONT_PORCH, V_SYNC_WIDTH, vtotal);
	if (INTERLACED || (SYNC_TYPE == SYNC_SEPARATE))
		fh2m_inno_seq_printf(m, "\t\t Sync Profile\t%s\"%sHSync\" \"%sVSync\"\n", INTERLACED ? "\"Interlace\" ": "",
				HSYNC_POSITIVE ? "+": "-", VSYNC_POSITIVE ? "+": "-");
	fh2m_inno_seq_printf(m, "\tEndMode\n");

	return 0;
}

static void DtdParse(struct seq_file *m, unsigned char *i_pDtd)
{
	int s32Vic = 0;

	s32Vic = Dtd2Vic(m, i_pDtd);
	fh2m_inno_seq_printf(m, "\tDTD support timing: %s\n", ConvertTiming2Name(s32Vic));
}

static void AudioParse(struct seq_file *m, unsigned char *i_pTable, unsigned char i_u8Off)
{
	int i = 0;
	int j = 0;
	...
	for (i = 0; i < tAudioData.u8Count; i++) {
		...
		fh2m_inno_seq_printf(m, "\tSAD%d support audio format: %s, max channels: %d\n",
			i+1, AUDIO_FORMAT(tAudioData.aSAD[i].eAudioFormat), tAudioData.aSAD[i].u8MaxChannels);
		fh2m_inno_seq_printf(m, "\tSAD%d support sample rate:", i+1);

		for (j = 0; j < 8; j++) {
			if (tAudioData.aSAD[i].u8SampleRate & (1<<j))
				fh2m_inno_seq_printf(m, " %s", SAMPLE_RATE(1<<j));
		}

		fh2m_inno_seq_printf(m, "\n");
		fh2m_inno_seq_printf(m, "\tSAD%d byte3: 0x%02X\n", i+1, tAudioData.aSAD[i].u8Byte3);
	}
}

static void VideoParse(struct seq_file *m, unsigned char *i_pTable, unsigned char i_u8Off)
{
	int i = 0;
	unsigned char u8Len = 0;
	unsigned char u8Tmp;
	...
	for (i = 0; i < u8Len; i++) {
		...
		fh2m_inno_seq_printf(m, "\t%s\n", ConvertTiming2Name(tVideoData.aVic[i]));
	}
}

static void Vsdb1Parse(struct seq_file *m, unsigned char *i_pTable, unsigned char i_u8Off)
{
	int i = 0;
	...
	fh2m_inno_seq_printf(m, "\tCEC phyaddr: 0x%02x 0x%02x\n", tVsdb1.u8PhyAddr0, tVsdb1.u8PhyAddr1);

	if (u8Len > 5) {
		tVsdb1.u8DcSupport = i_pTable[i_u8Off+6];
		if (tVsdb1.u8DcSupport & 0x08) {
			fh2m_inno_seq_printf(m, "\tsupport deep color type:");
			if (tVsdb1.u8DcSupport & 0x10)
				fh2m_inno_seq_printf(m, " %s", DEEP_COLOR(HDMI_DEEP_COLOR_10BIT));
			if (tVsdb1.u8DcSupport & 0x20)
				fh2m_inno_seq_printf(m, " %s", DEEP_COLOR(HDMI_DEEP_COLOR_12BIT));
			if (tVsdb1.u8DcSupport & 0x40)
				fh2m_inno_seq_printf(m, " %s", DEEP_COLOR(HDMI_DEEP_COLOR_16BIT));
		}
		fh2m_inno_seq_printf(m, "\n");
		if (u8Len == 6)
			return;
		...
		fh2m_inno_seq_printf(m, "\tsupport max tmds clock: %dMHz\n", tVsdb1.u32MaxTmdsClock);
		if (u8Len == 7)
			return;
		...
		if (tVsdb1.u8CnAndPresent & 0x80) {							// Latency_Fields_Present
			...
			u8NextOffset += 2;
			fh2m_inno_seq_printf(m, "\tVideo Latency: %dms\n", tVsdb1.u8VideoLatency);
			fh2m_inno_seq_printf(m, "\tAudio Latency: %dms\n", tVsdb1.u8AudioLatency);
		}
		if (tVsdb1.u8CnAndPresent & 0x40) {							// I_Latency_Fields_Present
			...
			u8NextOffset += 2;
			fh2m_inno_seq_printf(m, "\tI Video Latency: %dms\n", tVsdb1.u8IVideoLatency);
			fh2m_inno_seq_printf(m, "\tI Audio Latency: %dms\n", tVsdb1.u8IAudioLatency);
		}
		if (tVsdb1.u8CnAndPresent & 0x20) {							// HDMI_Video_present
			...
			fh2m_inno_seq_printf(m, "\tHDMI 4K VIC len: %d\n", tVsdb1.u8HdmiVicLen);
			...
			if (tVsdb1.u8Hdmi3dLen > 0) {
				...
				if (((u8Tmp & 0x60) == 0x20) || ((u8Tmp & 0x60) == 0x40)) {				// 3D_Multi_present
					...
					fh2m_inno_seq_printf(m, "\t3D support format:\n");
					for (i = 0; i < 16; i++) {
						if (u16Tmp & (1<<i))
							fh2m_inno_seq_printf(m, "\t\t %s\n", HDMI_3D_FORMAT(1<<i));
					}
				...
				for (i = 0; i < u8RemainingBytes; i++) {
					...
					fh2m_inno_seq_printf(m, "\t3D order: %04x\n", tVsdb1.u163dOrder[u83dOrderLen]);
...
static void Vsdb2Parse(struct seq_file *m, unsigned char *i_pTable, unsigned char i_u8Off)
{
	unsigned char u8Tmp;
	fh2m_inno_seq_printf(m, "\nHF-VSDB\n");
	...
	fh2m_inno_seq_printf(m, "\tSCDC present: %d\n", tVsdb2.u8ScdcPresent);
	...
	if (tVsdb2.u8Dc16Bit420)
		fh2m_inno_seq_printf(m, "\tsupport DC_48bit_420\n");
	if (tVsdb2.u8Dc12Bit420)
		fh2m_inno_seq_printf(m, "\tsupport DC_36bit_420\n");
	if (tVsdb2.u8Dc10Bit420)
		fh2m_inno_seq_printf(m, "\tsupport DC_30bit_420\n");
}

static void ExtTagParse(struct seq_file *m, unsigned char *i_pTable, unsigned char i_u8Off)
{
	int i = 0;
	int j = 0;
	...
	if (u8Tag == CEA_EXT_TAG_CDB) {				// Colorimetry Data Block
		...
		fh2m_inno_seq_printf(m, "\tsupport colorimetry:\n");
		for (i = 0; i < 8; i++) {
			if (u8Tmp & (1<<i))
				fh2m_inno_seq_printf(m, "\t\t %s\n", HDMI_COLORIMETRY(1<<i));
		}
		u8Tmp = i_pTable[i_u8Off+3];
		fh2m_inno_seq_printf(m, "\tsupport metadata profiles:\n");
		if(u8Tmp & 0x01) fh2m_inno_seq_printf(m, "\t\t %s\n", "Metadata Profile 0");
		if(u8Tmp & 0x02) fh2m_inno_seq_printf(m, "\t\t %s\n", "Metadata Profile 1");
		if(u8Tmp & 0x04) fh2m_inno_seq_printf(m, "\t\t %s\n", "Metadata Profile 2");
		if(u8Tmp & 0x08) fh2m_inno_seq_printf(m, "\t\t %s\n", "Metadata Profile 3");
	} else if (u8Tag == CEA_EXT_TAG_HDR) {					// HDR
		fh2m_inno_seq_printf(m, "\nHDR Static Metadata Data Block\n");
		// support HDR
		aHdrContent[0] = i_pTable[i_u8Off+2];
		aHdrContent[1] = i_pTable[i_u8Off+3];
		fh2m_inno_seq_printf(m, "\tsupport EOTF:\n");
...
void ParseEdidBlock0(struct seq_file *m, unsigned char *i_pEdid)
{
	int i = 0, j = 0;
	int k;
	unsigned char *block;
	...
	fh2m_inno_seq_printf(m, "\tManufacturer Name: %c%c%c\n", ManufacturerName[0], ManufacturerName[1], ManufacturerName[2]);

	ProductCode = i_pEdid[0x0B]<<8|i_pEdid[0x0A];
	fh2m_inno_seq_printf(m, "\tProduct Code: %04X\n", ProductCode);

	SerialNumber = i_pEdid[0x0F]<<24|i_pEdid[0x0E]<<16|i_pEdid[0x0D]<<8|i_pEdid[0x0C];
	fh2m_inno_seq_printf(m, "\tSerial Number: %08X\n", SerialNumber);

	WeekManufacturer = i_pEdid[0x10];
	fh2m_inno_seq_printf(m, "\tWeek of Manufacture: %d\n", WeekManufacturer);

	YearManufacturer = (i_pEdid[0x11]+1990);
	fh2m_inno_seq_printf(m, "\tYear of Manufacture: %d\n", YearManufacturer);

	fh2m_inno_seq_printf(m, "\nEDID Structure\n");
	fh2m_inno_seq_printf(m, "\tVersion no.: %d\n", i_pEdid[0x12]);
	fh2m_inno_seq_printf(m, "\tRevision no.: %d\n", i_pEdid[0x13]);

	fh2m_inno_seq_printf(m, "\nBasic Display Parameters and Features\n");
	fh2m_inno_seq_printf(m, "\tVideo Input Signal Type: %s\n", (i_pEdid[0x14]>>7) ? "Digital": "Analog");

	if (i_pEdid[0x14]>>7)			// Digital
		fh2m_inno_seq_printf(m, "\tInterface signal compatible with VESA DFP 1.X: %s\n", 
					(i_pEdid[0x14]&0x01) ? "Compatible": "Not Compatible");

	fh2m_inno_seq_printf(m, "\tMax Horz Size (in cm): %d\n", i_pEdid[0x15]);
	fh2m_inno_seq_printf(m, "\tMax Vert Size (in cm): %d\n", i_pEdid[0x16]);

	if (i_pEdid[0x17] == 0xFF)
		fh2m_inno_seq_printf(m ,"\tGamma Value: Not defined\n");
	else
		fh2m_inno_seq_printf(m, "\tGamma Value: %d.%d\n", (i_pEdid[0x17]+100)/100, (i_pEdid[0x17]+100)%100);


	fh2m_inno_seq_printf(m, "\tFeature Support (DPMS)\n");
	fh2m_inno_seq_printf(m, "\t\t Standby Mode: %s\n", (i_pEdid[0x18]&0x80) ? "Supported": "Not Supported");
	fh2m_inno_seq_printf(m, "\t\t Suspend Mode: %s\n", (i_pEdid[0x18]&0x40) ? "Supported": "Not Supported");
	fh2m_inno_seq_printf(m, "\t\t Active Off Mode: %s\n", (i_pEdid[0x18]&0x20) ? "Supported": "Not Supported");
	fh2m_inno_seq_printf(m, "\t\t Display Type: ");

	switch ((i_pEdid[0x18]&0x18)>>3) {
	case 0:
		fh2m_inno_seq_printf(m, "Monochrome display\n");
		break;
	case 1:
		fh2m_inno_seq_printf(m, "RGB color display\n");
		break;
	case 2:
		fh2m_inno_seq_printf(m, "Non-RGB multicolor display\n");
		break;
	case 3:
		fh2m_inno_seq_printf(m, "Undefined display\n");
		break;
	}

	fh2m_inno_seq_printf(m, "\t\t Color Space: %s\n", (i_pEdid[0x18]&0x04) ? "sRGB": "Alternate");
	fh2m_inno_seq_printf(m, "\t\t Preferred Timing: %s\n", (i_pEdid[0x18]&0x02) ? "1st DTD": "Non indicated");
	fh2m_inno_seq_printf(m, "\t\t GTF Timing: %s\n", (i_pEdid[0x18]&0x01) ? "Supported": "Not Supported");
	fh2m_inno_seq_printf(m, "\nColor Characteristic\n");
	fh2m_inno_seq_printf(m, "\tRed_x: 0.%.3d\n", ((i_pEdid[0x1B]<<2)	| ((i_pEdid[0x19]>>6)&0x03))*1000/1024);
	fh2m_inno_seq_printf(m, "\tRed_y: 0.%.3d\n", ((i_pEdid[0x1C]<<2)	| ((i_pEdid[0x19]>>4)&0x03))*1000/1024);
	fh2m_inno_seq_printf(m, "\tGreen_x: 0.%.3d\n", ((i_pEdid[0x1D]<<2)  | ((i_pEdid[0x19]>>2)&0x03))*1000/1024);
	fh2m_inno_seq_printf(m, "\tGreen_y: 0.%.3d\n", ((i_pEdid[0x1E]<<2) | (i_pEdid[0x19]&0x03))*1000/1024);
	fh2m_inno_seq_printf(m, "\tBlue_x: 0.%.3d\n", ((i_pEdid[0x1F]<<2)  | ((i_pEdid[0x1A]>>6)&0x03))*1000/1024);
	fh2m_inno_seq_printf(m, "\tBlue_y: 0.%.3d\n", ((i_pEdid[0x20]<<2)  | ((i_pEdid[0x1A]>>4)&0x03))*1000/1024);
	fh2m_inno_seq_printf(m, "\tWhite_x: 0.%.3d\n", ((i_pEdid[0x21]<<2)  | ((i_pEdid[0x1A]>>2)&0x03))*1000/1024);
	fh2m_inno_seq_printf(m, "\tWhite_y: 0.%.3d\n", ((i_pEdid[0x22]<<2)  | (i_pEdid[0x1A]&0x03))*1000/1024);


	fh2m_inno_seq_printf(m, "\nEstablished Timings\n");
	fh2m_inno_seq_printf(m, "\tEstablished Timings I \n");
	if (i_pEdid[0x23]&0x80)
		fh2m_inno_seq_printf(m, "\t\t 720 x 400 @ 70Hz IBM, VGA\n");
	if (i_pEdid[0x23]&0x40)
		fh2m_inno_seq_printf(m, "\t\t 720 x 400 @ 88Hz IBM, XGA2\n");
	if (i_pEdid[0x23]&0x20)
		fh2m_inno_seq_printf(m, "\t\t 640 x 480 @ 60Hz IBM, VGA\n");
	if (i_pEdid[0x23]&0x10)
		fh2m_inno_seq_printf(m, "\t\t 640 x 480 @ 67Hz Apple, Mac II\n");
	if (i_pEdid[0x23]&0x08)
		fh2m_inno_seq_printf(m, "\t\t 640 x 480 @ 72Hz VESA\n");
	if (i_pEdid[0x23]&0x04)
		fh2m_inno_seq_printf(m, "\t\t 640 x 480 @ 75Hz VESA\n");
	if (i_pEdid[0x23]&0x02)
		fh2m_inno_seq_printf(m, "\t\t 800 x 600 @ 56Hz VESA\n");
	if (i_pEdid[0x23]&0x01)
		fh2m_inno_seq_printf(m, "\t\t 800 x 600 @ 60Hz VESA\n"); 
	if (i_pEdid[0x23]== 0x00)
		fh2m_inno_seq_printf(m, "\t\t None\n");

	fh2m_inno_seq_printf(m, "\tEstablished Timings II \n");
	if (i_pEdid[0x24]&0x80)
		fh2m_inno_seq_printf(m, "\t\t 800 x 600 @ 72Hz VESA\n");
	if (i_pEdid[0x24]&0x40)
		fh2m_inno_seq_printf(m, "\t\t 800 x 600 @ 75Hz VESA\n");
	if (i_pEdid[0x24]&0x20)
		fh2m_inno_seq_printf(m, "\t\t 832 x 624 @ 75Hz Apple, Mac II\n");
	if (i_pEdid[0x24]&0x10)
		fh2m_inno_seq_printf(m, "\t\t 1024 x 768 @ 87Hz IBM\n");
	if (i_pEdid[0x24]&0x08)
		fh2m_inno_seq_printf(m, "\t\t 1024 x 768 @ 60Hz VESA\n");
	if (i_pEdid[0x24]&0x04)
		fh2m_inno_seq_printf(m, "\t\t 1024 x 768 @ 70Hz VESA\n");
	if (i_pEdid[0x24]&0x02)
		fh2m_inno_seq_printf(m, "\t\t 1024 x 768 @ 75Hz VESA\n");
	if (i_pEdid[0x24]&0x01)
		fh2m_inno_seq_printf(m, "\t\t 1280 x 1024 @ 75Hz VESA\n");
	if (i_pEdid[0x24]== 0x00)
		fh2m_inno_seq_printf(m, "\t\t None\n");

	fh2m_inno_seq_printf(m, "\tManufacturer's Timings\n");
	if (i_pEdid[0x25]&0x80)
		fh2m_inno_seq_printf(m, "\t\t 1152 x 870 @ 75Hz Apple, Mac II\n");
	else
		fh2m_inno_seq_printf(m, "\t\t None\n");

	fh2m_inno_seq_printf(m, "\nStandard Timings\n");
	for (i = 0; i < 8; i++) {
		fh2m_inno_seq_printf(m, "\tStandard Timing %d\n", i+1);
		if ((i_pEdid[0x26+i*2] == 0x01) && (i_pEdid[0x26+i*2+1] == 0x01))
			fh2m_inno_seq_printf(m, "\t\t Unused\n");
		else {
			unsigned int HActive = (i_pEdid[0x26+i*2]+31)*8;
			unsigned int VActive = 0;
			unsigned char u8Rate = 0;
			fh2m_inno_seq_printf(m, "\t\t Horizontal active pixels: %d\n", HActive);
			fh2m_inno_seq_printf(m, "\t\t Image Aspect ratio: ");
			switch ((i_pEdid[0x26+i*2+1]&0xC0)>>6) {
			case 0:
				fh2m_inno_seq_printf(m, "16:10\n");
				VActive = HActive * 10 / 16;
				break;
			case 1:
				fh2m_inno_seq_printf(m, "4:3\n");
				VActive = HActive * 3 / 4;
				break;
			case 2:
				fh2m_inno_seq_printf(m, "5:4\n");
				VActive = HActive * 4 / 5;
				break;
			case 3:
				fh2m_inno_seq_printf(m, "16:9\n");
				VActive = HActive * 9 / 16;
				break;
			}
			u8Rate = (i_pEdid[0x26+i*2+1]&0x3F)+60;
			fh2m_inno_seq_printf(m, "\t\t Refresh Rate: %d Hz\n", u8Rate);
		}
	}
	...
	for (i = 0; i < BLOCK0_TOTAL_DTD; i++, block += DTD_SIZE) {
		...
		if (i == 0 )
			fh2m_inno_seq_printf(m, "--------prefered timing mode \r\n");
		...
		case MONITOR_NAME:
			...
			fh2m_inno_seq_printf(m, "\tMonitor name: %s\n", ProductName);
			break;
		case RANGE_LIMITS:
			fh2m_inno_seq_printf(m, "\tMin. Vertical rate (Hz): %d\n", block[5]);
			fh2m_inno_seq_printf(m, "\tMax. Vertical rate (Hz): %d\n", block[6]);
			fh2m_inno_seq_printf(m, "\tMin. Horizontal rate (KHz): %d\n", block[7]);
			fh2m_inno_seq_printf(m, "\tMax. Horizontal rate (KHz): %d\n", block[8]);
			fh2m_inno_seq_printf(m, "\tMax. Supported Pixel Clock rate (KHz): %d\n", block[9]*10);
			if(block[10] == 0x00)
				fh2m_inno_seq_printf(m, "\tDefault GTF supported\n");
			else if(block[10] == 0x02)
				fh2m_inno_seq_printf(m, "\tSecondary GTF supported\n");
			break;
		case PRODUCT_SERIAL_NUMBER:
			fh2m_inno_seq_printf(m, "\tProduct Serial Number: ");
			for (k = 0; k < 13; k++) {
				fh2m_inno_seq_printf(m, "%c", block[k+5]);
				if (block[k+5] == 0x0A)
					break;
			}
		default:
...
static int
inno_cea_db_offsets(const u8 *cea, int *start, int *end)
{
...
static int inno_cea_db_payload_len(const u8 *db)
{
	return db[0] & 0x1f;
}

static int inno_cea_db_tag(const u8 *db)
{
	return db[0] >> 5;
}

#define inno_for_each_cea_db(cea, i, start, end) \
	for ((i) = (start); (i) < (end) && (i) + inno_cea_db_payload_len(&(cea)[(i)]) < (end); (i) += inno_cea_db_payload_len(&(cea)[(i)]) + 1)

unsigned char ParseEdidBlock1(struct seq_file *m, unsigned char *i_pEdid)
{
	int i = 0;
	int j = 0;
	u8 aDtd[18] = {0};
	...
	inno_for_each_cea_db(i_pEdid, i, start, end) {
		...
		switch (db_type) {
		case CEA_TAG_ADB:
			AudioParse(m, i_pEdid, i);
			break;
		case CEA_TAG_VDB:
			VideoParse(m, i_pEdid, i);
			break;
		case CEA_TAG_VSDB:
			if ((i_pEdid[i+1] == 0x03) && (i_pEdid[i+2] == 0x0C) && (i_pEdid[i+3] == 0x00))
				Vsdb1Parse(m, i_pEdid, i);
			else if ((i_pEdid[i+1] == 0xD8) && (i_pEdid[i+2] == 0x5D) && (i_pEdid[i+3] == 0xC4))
				Vsdb2Parse(m, i_pEdid, i);
			break;
		case CEA_TAG_SPK:
			u8SpeakerInfo = i_pEdid[i+1];
			fh2m_inno_seq_printf(m, "\nSpeaker Allocation Data Block\n");
			fh2m_inno_seq_printf(m, "\tspeaker info: 0x%02X\n", u8SpeakerInfo);
			break;
		case CEA_TAG_EXT:
			ExtTagParse(m, i_pEdid, i);
		default:
			break;
		}
	}
