...
int main(void)
{
    ...
    PFNEGLGETDISPLAYPROC eglGetDisplay = require_symbol(egl, "eglGetDisplay");
    ...
    if (!eglGetProcAddress || !eglGetDisplay || !eglGetError || !eglInitialize || !eglBindAPI ||
    ...
    if (eglGetPlatformDisplayEXT) {
        dpy = eglGetPlatformDisplayEXT(EGL_PLATFORM_X11_EXT, xdpy, NULL);
        printf("eglGetPlatformDisplayEXT(X11) = %p\n", (void *)dpy);
    } else {
        dpy = eglGetDisplay((EGLNativeDisplayType)xdpy);
        printf("eglGetDisplay(X11) = %p\n", (void *)dpy);
    }
    if (dpy == EGL_NO_DISPLAY) {
        fprintf(stderr, "EGL_NO_DISPLAY: 0x%x\n", eglGetError());
        XCloseDisplay(xdpy);
        return 1;
    }
