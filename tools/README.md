...
| `audit-licenses.py` | 源码/载荷许可证机械审计 | 按 `license-audit-policy.json` 枚举 Git 跟踪的 `drivers/` 路径，严格识别完整 Dual MIT/GPLv2、confidential、BSD/LGPL、标准 SPDX 与 `NOASSERTION`，校验许可证文本 hash、修改路径 patch provenance 和 192 项 manifest license 证据，确定性生成/检查 `source-license-inventory.tsv`。普通检查可在 `license_release_gate=BLOCKED` 时 PASS；release 必须用 `--require-releasable`，当前预期 rc=2。该工具不作法律结论 |
