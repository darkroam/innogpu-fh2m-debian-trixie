...
W=$(mktemp -d /tmp/innogpu-pkg9.XXXXXX)
...
import sys; p=Path(sys.argv[1]); s=p.read_text()
s=s.replace('Version: 3.3.3.42-patched-6','Version: 3.3.3.42-patched-9')
p.write_text(s)
PY
...
from pathlib import Path
import sys; p=Path(sys.argv[1]); s=p.read_text()
s=s.replace('innogpu-fh2m-trixie postinst (patched-6)', 'innogpu-fh2m-trixie postinst (patched-9)')
...
'''
repl='''if [ -x /usr/share/innogpu-fh2m-trixie/disable-incompatible-userspace.sh ]; then
    ln -sf ../share/innogpu-fh2m-trixie/disable-incompatible-userspace.sh /usr/sbin/innogpu-disable-incompatible-userspace
    ln -sf ../share/innogpu-fh2m-trixie/disable-incompatible-userspace.sh /usr/bin/innogpu-disable-incompatible-userspace
fi
if [ -x /usr/share/innogpu-fh2m-trixie/install-kylin-userspace.sh ]; then
...
dpkg-deb --root-owner-group --build "$W/root" innogpu-fh2m-trixie_3.3.3.42-patched-9.deb
echo "Built: innogpu-fh2m-trixie_3.3.3.42-patched-9.deb"
