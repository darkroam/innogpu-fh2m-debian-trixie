...
| `build-innogpu-driver.sh` | **新架构当前构建器** | 默认 `4.0.2-i3`；R27 F 候选只许 `5.0.0-i8` + `SOURCE_DATE_EPOCH=1790121600`：校验冻结 i6 meta/snapshot/tree，再仅追加 cfg_detect 输出排序，派生树锁定 `2c1c45c5d4ef`。编译/包内源共用派生；父树trace与派生整树门分列，i1-i7/未知版本/错 epoch 拒绝。F 并行度最多8，共用ABI门，O配方保持；不表示安装或R5通过 |
