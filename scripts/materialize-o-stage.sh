#!/usr/bin/env bash
# scripts/materialize-o-stage.sh — 阶段三 O_stage 源树物化（F0 + 17 条 030-NNN → o-stage 快照）
#
# 契约：docs/planning/o-stage-integration-plan.md §一（dsh 终审通过）。
#   - 前置校验（tar 1.35 / zstd 1.5.7 / f0 快照 SHA / 17 条 patch SHA）
#   - 解包 F0 → 树 hash 校验（O-4 锁定 7219d817…）
#   - 17 条 030-NNN 按链序 -p1 --fuzz=0 应用，每链点校验 after_tree_hash
#   - 最终树 hash == 030-033.meta.json after_tree_hash（= 4be9ba75…）
...
# docs/planning/evidence/o-stage/5.0.0-i5，候选代隔离目录）与 /tmp 工作目录；
# 绝不写任何保护区路径（debs/vendor/build/third_party 及 drivers/baselines）。
#
# 用法：scripts/materialize-o-stage.sh（无参数；不接受任何参数；版本族经
# OSTAGE_VERSION/OSTAGE_TAG/OSTAGE_META_NAME/OSTAGE_VALIDATION_RESULTS 注入，缺省 5.0.0-i5 代）
...
FINAL_TREE_HASH_DEFAULT="4be9ba7509258726b95bd41137da1280868c91c835aa57d4150fd6ba76913300"
...
# ---- 版本/provenance（参数化；默认 5.0.0-i5 代——030-033 ABI 修正代。
# i2/i3/i4 五件产物是 R5 失败档案锚点，不得覆盖；本代生成独立 i5 meta） ----
VERSION="${OSTAGE_VERSION:-5.0.0-i5}"
TAG="${OSTAGE_TAG:-fantgpu-5.0.0-i5}"
SOURCE_DATE_EPOCH="${OSTAGE_SOURCE_DATE_EPOCH:-1789516800}"
VALIDATION_RESULTS="${OSTAGE_VALIDATION_RESULTS:-docs/planning/evidence/o-stage/5.0.0-i5-validation-results.json}"
VALIDATION_SCHEMA="validation-results-1.0"
META_NAME="${OSTAGE_META_NAME:-5.0.0-i5.meta.json}"

# ---- 路径 ----
IN_DIR="${OSTAGE_INPUT_DIR:-$ROOT/docs/planning/evidence/o-stage}"
PATCH_DIR="${OSTAGE_PATCH_DIR:-$ROOT/patches}"
OUT_DIR="${OSTAGE_OUT_DIR:-$ROOT/docs/planning/evidence/o-stage/5.0.0-i5}"
...
VERSION = os.environ.get("OSTAGE_VERSION") or "5.0.0-i5"
TAG = os.environ.get("OSTAGE_TAG") or "fantgpu-5.0.0-i5"
SDE = os.environ.get("OSTAGE_SOURCE_DATE_EPOCH") or "1789516800"
VAL_RES = (os.environ.get("OSTAGE_VALIDATION_RESULTS")
           or "docs/planning/evidence/o-stage/5.0.0-i5-validation-results.json")
VAL_SCHEMA = "validation-results-1.0"
META_NAME = os.environ.get("OSTAGE_META_NAME") or "5.0.0-i5.meta.json"
...
def artifacts_self_consistent():
    ...
    # journal 未更新的状态判定依据）
    try:
        with open(dest[META_NAME], "r", encoding="utf-8") as fh:
            m = json.load(fh)
...
try:
    ...
    with open(txn[META_NAME], "w", encoding="utf-8") as fh:
...
# 17 条 patch + SHA 校验（与 meta.json apply.patch_sha256 一致）
...
# 17 条链序应用 + 每链点 after_tree_hash 校验
...
last_after="$(meta_field "$PATCH_DIR/030-033.meta.json" "apply.after_tree_hash")"
...
OSTAGE_VERSION="$VERSION" OSTAGE_TAG="$TAG" \
OSTAGE_SOURCE_DATE_EPOCH="$SOURCE_DATE_EPOCH" \
OSTAGE_VALIDATION_RESULTS="$VALIDATION_RESULTS" \
OSTAGE_META_NAME="$META_NAME" \
