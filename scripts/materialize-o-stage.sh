...
#   - 最终树 hash == 030-034.meta.json after_tree_hash（= 5f6a5347…）
...
# docs/planning/evidence/o-stage/5.0.0-i6，候选代隔离目录）与 /tmp 工作目录；
# 绝不写任何保护区路径（debs/vendor/build/third_party 及 drivers/baselines）。
#
# 用法：scripts/materialize-o-stage.sh（无参数；不接受任何参数；版本族经
# OSTAGE_VERSION/OSTAGE_TAG/OSTAGE_META_NAME/OSTAGE_VALIDATION_RESULTS 注入，缺省 5.0.0-i6 代）
...
FINAL_TREE_HASH_DEFAULT="5f6a5347c7e217ba3f7c5b71fdcad520148e0bc71231ab95fb023655865d11da"
...
# ---- 版本/provenance（参数化；默认 5.0.0-i6 代——030-034 stop-stage 诊断代。
# 既有各代五件产物不得覆盖；本代生成独立 i6 meta） ----
VERSION="${OSTAGE_VERSION:-5.0.0-i6}"
TAG="${OSTAGE_TAG:-fantgpu-5.0.0-i6}"
SOURCE_DATE_EPOCH="${OSTAGE_SOURCE_DATE_EPOCH:-1789516800}"
VALIDATION_RESULTS="${OSTAGE_VALIDATION_RESULTS:-docs/planning/evidence/o-stage/5.0.0-i6-validation-results.json}"
VALIDATION_SCHEMA="validation-results-1.0"
META_NAME="${OSTAGE_META_NAME:-5.0.0-i6.meta.json}"

# ---- 路径 ----
IN_DIR="${OSTAGE_INPUT_DIR:-$ROOT/docs/planning/evidence/o-stage}"
PATCH_DIR="${OSTAGE_PATCH_DIR:-$ROOT/patches}"
OUT_DIR="${OSTAGE_OUT_DIR:-$ROOT/docs/planning/evidence/o-stage/5.0.0-i6}"
...
VERSION = os.environ.get("OSTAGE_VERSION") or "5.0.0-i6"
TAG = os.environ.get("OSTAGE_TAG") or "fantgpu-5.0.0-i6"
SDE = os.environ.get("OSTAGE_SOURCE_DATE_EPOCH") or "1789516800"
VAL_RES = (os.environ.get("OSTAGE_VALIDATION_RESULTS")
           or "docs/planning/evidence/o-stage/5.0.0-i6-validation-results.json")
VAL_SCHEMA = "validation-results-1.0"
META_NAME = os.environ.get("OSTAGE_META_NAME") or "5.0.0-i6.meta.json"
...
# 18 条 patch + SHA 校验（与 meta.json apply.patch_sha256 一致）
...
# 18 条链序应用 + 每链点 after_tree_hash 校验
...
last_after="$(meta_field "$PATCH_DIR/030-034.meta.json" "apply.after_tree_hash")"
