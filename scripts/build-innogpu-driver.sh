...
# 血统：5.0.0-iN = fantgpu 血统（O_stage 物化源树；i6 为 030 链 18 项，
...
if digest() != "e4d043bff8ed79b92b52512aad8f8e05ac51113bef2dd4cb4505dd96c510ccb7":
    sys.exit("builder_derived_source=FAIL i9 intermediate tree drift")
# Each label unwinds only operations that succeeded. Bluetooth never registers
# a PM notifier; no extra state or shared dev_rsrc layout change is needed.
path = tree / "fantpower/fant_input_event.c"
text = path.read_text()
changes = [
    ("\tif (!input_handle_priv)\n\t\treturn -ENOMEM;",
     "\tif (!input_handle_priv) {\n\t\terror = -ENOMEM;\n\t\tgoto err_free_handle;\n\t}"),
    ("\t\tregister_pm_notifier(&input_handle_priv->pm_nb);",
     "\t\terror = register_pm_notifier(&input_handle_priv->pm_nb);\n\t\tif (error)\n\t\t\tgoto err_free_private;"),
...
if [[ "$FANT_LINEAGE" == 1 ]]; then
	# fantgpu 血统：DKMS 源 = O_stage 物化快照解包（030 链 18 项已含——
	# 030-030 音频回退、030-031 PM marker、030-032 探针、030-033 ABI 修正及 030-034 stop-stage 随链入树，patch-000 no-transform、o_shipped
	...
	OSTAGE_DIR="$ROOT/docs/planning/evidence/o-stage/5.0.0-i6"
    OSTAGE_SNAP="$OSTAGE_DIR/o-stage-snapshot.tar.zst"
    [[ -f "$OSTAGE_SNAP" ]] || { echo "staging_ostage_snapshot=FAIL"; exit 1; }
	OSTAGE_TREE_HASH="5f6a5347c7e217ba3f7c5b71fdcad520148e0bc71231ab95fb023655865d11da"
	...
	meta_sha="$(python3 - "$OSTAGE_DIR/5.0.0-i6.meta.json" <<'PY'
import json, sys
...
if [[ "$FANT_LINEAGE" == 1 ]]; then
    ...
    # F wrapper 防护在 O_stage payload trace 校验完成后应用，避免把明确登记的
    # F-only 适配误报为 O_stage provenance 漂移。
else
