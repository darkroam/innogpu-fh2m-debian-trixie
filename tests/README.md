...
- suspend/resume 静态测试把跟踪的 `pvr_dvfs_device.c` 与 `innodpu_drm_pm.c` 复制到 `/tmp`，验证
  patch-024/patch-025-suspend-resume-display dry-run/应用、PowerLock 门禁顺序、post-atomic 重复光标
  恢复移除、单文件范围、`4.0.1-i1`/`4.0.1-i2` 版本与固定 epoch 失败关闭（含旧版本、错误 epoch 和
  未审核迭代反例）、编译树与包源码双应用及 patched-28 legacy 开关；不读取本地载荷、不构建或安装驱动、
  不挂起主机；
- 许可证审计测试覆盖当前逐文件 inventory 一致性、发布门禁保持 BLOCKED、确定性重建、
  ...
  `drivers/` 或发布状态。
- 版本测试断言 `4.0.1-i2 > 4.0.1-i1 > 4.0.0-i1 > patched-27`、`1.0.0-i1 < patched-27` 等排序契约；
