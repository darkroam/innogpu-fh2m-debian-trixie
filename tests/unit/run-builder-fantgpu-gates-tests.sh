...
# §四（postinst/md5sums/trace）：当前版本 5.0.0-i4、血统判定 5.0.0-i*、
...
# t02 5.0.0-i4 通过 allowlist + epoch 门 → 死在 kernel headers（证明版本/血统被接受）
run_builder VERSION=5.0.0-i4 SOURCE_DATE_EPOCH=1788796800
...
run_builder VERSION=5.0.0-i4 SOURCE_DATE_EPOCH=1111111111
...
if grep -Fq 'PKG_DESC="Innosilicon Fantasy II-M driver (fantgpu lineage $VERSION, O_stage materialized tree)"' "$BUILDER" \
   && grep -Fq 'O_stage materialized source tree (030 chain of 16,' "$BUILDER" \
...
# t13 静态契约：O_stage meta 引用 i4 隔离代（030-032 入链后锁定新树 hash）
if grep -Fq 'OSTAGE_DIR="$ROOT/docs/planning/evidence/o-stage/5.0.0-i4"' "$BUILDER" \
   && grep -Fq '"$OSTAGE_DIR/5.0.0-i4.meta.json"' "$BUILDER" \
   && grep -Fq 'OSTAGE_TREE_HASH="43f63f3fd5a9305734b9749cdfe773bc303726b06bf9f94fc1b55347738e9213"' "$BUILDER"; then
    ok t13
else
...
# 补丁——F 包内 DKMS 源 = 16 链 O_stage 快照本身（030-032 已随链入树），
# 补丁后状态由锁定 o_stage_tree_hash 覆盖；无 apply_fantgpu_runtime_fixes、
# 无顶层非法补丁名引用。
if grep -Fq 'APPLIED_SOURCE_FIXES="o-stage-materialized-030-chain-16' "$BUILDER" \
   && ! grep -Fq 'apply_fantgpu_runtime_fixes' "$BUILDER" \
   && ! grep -Fq 'fantgpu-hwinfo-audio-fallback.patch' "$BUILDER"; then
    ok t15
else
    bad t15 "post-trace patching not removed or 16-chain wiring missing"
fi
...
# t17 入链印证：锁定的 O_stage 快照必须已含 030-032——reverse dry-run 成功
# 证明补丁已随链入树且可干净反转（正向 dry-run 必然失败：已应用）。
PATCH_TREE="$TMP/patch-tree"
mkdir -p "$PATCH_TREE"
if tar --use-compress-program=zstd -xf \
       "$ROOT/docs/planning/evidence/o-stage/5.0.0-i4/o-stage-snapshot.tar.zst" -C "$PATCH_TREE" \
   && patch --batch --forward --fuzz=0 --no-backup-if-mismatch --dry-run -R -s \
       -d "$PATCH_TREE/o-stage" -p1 < "$ROOT/patches/030-032.patch"; then
    ok t17
else
    bad t17 "030-032 not present in locked O_stage snapshot (reverse dry-run failed)"
fi
