...
# 假 chain 文件在 materialize 内由真实 14 条链驱动，假输入用例仅覆盖
...
for f in o-stage-snapshot.tar.zst o-stage-snapshot.tar.zst.sha256 \
         o-stage.manifest.tsv o-stage.manifest.tsv.sha256 5.0.0-i2.meta.json; do
...
sha256sum "$REAL_OUT/o-stage-snapshot.tar.zst" "$REAL_OUT/5.0.0-i2.meta.json" > "$TMP/run1.sha"
set +e
env OSTAGE_OUT_DIR="$REAL_OUT" OSTAGE_WORK_DIR="$REAL_WORK" \
...
sha256sum "$REAL_OUT/o-stage-snapshot.tar.zst" "$REAL_OUT/5.0.0-i2.meta.json" > "$TMP/run2.sha"
